--[[
	...
	Originally Dex++ by Chillz, a revival of Moon's Dex v3.

	This is the application core. It is fetched and run by loader.lua, then it
	pulls each app module (Lib, Explorer, Properties, ...) over HTTP through the
	global `Axon.Import` helper the loader installed.

	The original Dex++ lived in one chunk where `game`, `oldgame`, `cloneref`,
	`nodes` and `selection` were shared lexically. Splitting into many files
	breaks that shared scope, so those five are now shared via globals (or a
	tiny per-file prelude); everything else still flows through the dependency
	table built by Main.GetInitDeps().
]]
...
-- Shared state used across multiple modules (Explorer owns these).
nodes = nodes or {}
selection = selection or nil
...
-- Main vars
local Main, Explorer, Properties, ScriptViewer, Console, SaveInstance, ModelViewer, SettingsWindow, ScriptTree, RemoteTree, AssetTree, ScriptSearch, FunctionDumper, EnvExplorer, DecompilerHelper, DefaultSettings, Notebook, Serializer, Lib local ggv = getgenv or nil
local API, RMD
	...
	return {
		...
		Theme = {
			...
			Syntax = {
				...
				SelectionBack = rgb(86,173,244),
				Operator = rgb(240,240,245),
				Number = rgb(255,198,0),
				String = rgb(173,241,149),
				Comment = rgb(120,120,125),
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","Console","SaveInstance","ModelViewer","SettingsWindow","ScriptTree","RemoteTree","AssetTree","ScriptSearch","FunctionDumper","EnvExplorer","DecompilerHelper"}
	Main.Elevated = false
	...
	Main.SecureGui = function(gui)
		gui.Name = "_AXON_".. Main.GetRandomString()
	...
	Main.Error = function(str)
		if rconsoleprint then
			rconsoleprint("AXON ERROR: "..tostring(str).."\n")
			wait(9e9)
		else
			error(str)
		end
	...
	-- Web loader: every module lives at src/Modules/<name>.lua and is fetched
	-- and memoised by the loader's Axon.Import.
	Main.LoadModule = function(name)
		local control = Axon.Import("src/Modules/"..name)
		if type(control) ~= "table" or not control.Main then
			Main.Error("Failed to load module: "..tostring(name))
		end
	...
	Main.LoadModules = function()
		...
		DecompilerHelper = Apps.DecompilerHelper
		...
		local appTable = {
			...
			DecompilerHelper = DecompilerHelper
		}
	...
	Main.IncompatibleTest = function()
		-- Reserved for executor-compatibility checks.
	end
	...
	Main.ExportSettings = function()
		...
		-- serialize color3 before encode
	...
	Main.LoadSettings = function()
		local s, data = pcall(env.readfile or error, "AxonSettings.json")
	...
	Main.CreateIntro = function(initStatus)
		local gui = create({
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-190,0,15),Size=UDim2.new(0,100,0,50),Text="Axon",TextColor3=Settings.Theme.Text,TextSize=50,TextTransparency=1,}},
			...
			{8,"Frame",{BackgroundColor3=Settings.Theme.Main2,BorderSizePixel=0,Name="ProgressBar",Parent={3},Position=UDim2.new(0,110,0,145),Size=UDim2.new(0,0,0,4),}},
			{9,"Frame",{BackgroundColor3=Settings.Theme.ListSelection,BorderSizePixel=0,Name="Bar",Parent={8},Size=UDim2.new(0,0,1,0),}},
			{10,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="rbxassetid://2764171053",ImageColor3=Settings.Theme.Main1,Parent={8},ScaleType=1,Size=UDim2.new(1,0,1,0),SliceCenter=Rect.new(2,2,254,254),}},
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Developed by Axon.",TextColor3=Settings.Theme.Text,TextSize=14,TextXAlignment=1,}},
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "Close" or "Axon"
	...
	Main.CreateMainGui = function()
		local gui = create({
			...
			{6,"Frame",{BackgroundColor3=Settings.Theme.Main2,BackgroundTransparency=Settings.Window.Transparency,Name="BottomFrame",Parent={4},Position=UDim2.new(0,0,1,-24),Size=UDim2.new(1,0,0,24),}},
			{7,"UICorner",{CornerRadius=UDim.new(0,4),Parent={6},}},
			{8,"Frame",{BackgroundColor3=Settings.Theme.Main2,BackgroundTransparency=Settings.Window.Transparency,BorderSizePixel=0,Name="CoverFrame",Parent={6},Size=UDim2.new(1,0,0,4),}},
			...
			{14,"ScrollingFrame",{Active=true,AnchorPoint=Vector2.new(0.5,0),BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,BorderColor3=Settings.Theme.Outline1,BorderSizePixel=0,Name="AppsFrame",Parent={4},Position=UDim2.new(0.5,0,0,0),ScrollBarImageColor3=Color3.new(0,0,0),ScrollBarThickness=4,Size=UDim2.new(0,222,1,-25),}},
			...
			{18,"TextButton",{AutoButtonColor=false,BackgroundColor3=Settings.Theme.Button,BackgroundTransparency=Settings.Window.Transparency,BorderSizePixel=0,Font=3,Name="Main",Parent={17},Size=UDim2.new(1,0,0,60),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
		...
		Main.CreateApp({Name = "Env Explorer", IconMap = Main.LargeIcons, Icon = "Output", Window = EnvExplorer.Window})
	...
	Main.SetupFilesystem = function()
		...
		makefolder("axon")
		makefolder("axon/assets")
		makefolder("axon/saved")
		makefolder("axon/plugins")
	end
	...
	Main.Init = function()
		...
		if Main.Elevated then
			local fileVer = Lib.ReadFile("axon/deps_version.dat")
		...
		local modulesToInit = {
			...
			SaveInstance, ModelViewer, SettingsWindow, RemoteTree, AssetTree, ScriptSearch, FunctionDumper, EnvExplorer, DecompilerHelper
		}
		...
		if env.readfile and listfiles then
			if #listfiles("axon/plugins") > 0 then
				...
				for _, pluginDir in pairs(listfiles("axon/plugins")) do
	...
	Main.Uninit = function()
		...
		for _, gui in pairs(Main.GetSecureContainer():GetChildren()) do
			if string.sub(gui.Name,1,6) == "_AXON_" then
				gui:Destroy()
			end
		end
	end
